...
import { ArrowLeft, Eraser, Lightbulb, RotateCcw, Pencil, Play, Pause, Trophy } from 'lucide-react';
...
const GameScreen: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Main Game Container */}
      <main className={`flex-1 flex flex-col px-5 pt-4 pb-24 overflow-y-auto transition-opacity duration-1000 ${isVictory ? 'opacity-30' : 'opacity-100'}`}>
        {/* Status Bar */}
        <div className="flex justify-between items-center bg-subtle-bg rounded-full px-4 py-2 mb-6 border border-border">
          <div className="flex flex-col items-start">
            <span className="font-hanken text-[10px] font-bold text-secondary uppercase tracking-wider">{t('game.time')}</span>
            <span className="font-hanken text-lg font-bold text-primary-text">{formatTime(timeElapsed)}</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="font-hanken text-[10px] font-bold text-secondary uppercase tracking-wider">{t('game.level')}</span>
            <span className="font-hanken text-lg font-bold text-error uppercase">{t(`main_menu.difficulties.${selectedDifficulty}`)}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="font-hanken text-[10px] font-bold text-secondary uppercase tracking-wider">{t('game.mistakes')}</span>
        ...
        <div className="relative">
          ...
          {isPaused && !isVictory && (
            <div 
              ...
              onKeyDown={(e) => e.key === 'Enter' && setPaused(false)}
              role="button"
              tabIndex={0}
            ...
            >
              ...
              <span className="font-hanken text-xl font-bold uppercase tracking-widest-premium">{t('game.paused')}</span>
        ...
        {/* Action Bar */}
        <div className={`grid grid-cols-4 gap-2 mb-6 transition-opacity ${isVictory ? 'pointer-events-none opacity-20' : ''}`}>
          <button type="button" className="flex flex-col items-center justify-center py-3 bg-white border border-border rounded-xl text-primary-text hover:bg-subtle-bg transition-all active:scale-95">
            <Eraser className="w-5 h-5 mb-1" />
            <span className="font-hanken text-[10px] font-bold tracking-wider uppercase">{t('game.erase')}</span>
          </button>
          <button 
            type="button"
            ...
            className={`flex flex-col items-center justify-center py-3 rounded-xl transition-all active:scale-95 ${
              isNoteMode ? 'bg-primary-text text-white shadow-md' : 'bg-white border border-border text-primary-text hover:bg-subtle-bg'
            }`}
          >
            <Pencil className={`w-5 h-5 mb-1 ${isNoteMode ? 'fill-current' : ''}`} />
            <span className="font-hanken text-[10px] font-bold tracking-wider uppercase">{t('game.notes')}</span>
          </button>
          <button 
            type="button"
            onClick={useHint}
          ...
          >
            ...
            <span className="font-hanken text-[10px] font-bold tracking-wider uppercase">{t('game.hint')} ({hintsUsed}/3)</span>
          </button>
          <button 
            type="button"
          ...
          >
            ...
            <span className="font-hanken text-[10px] font-bold tracking-wider uppercase">{t('game.restart')}</span>
          </button>
        </div>

        {/* Keypad */}
        <div className={`mt-auto transition-opacity ${isVictory ? 'pointer-events-none opacity-20' : ''}`}>
